...
read_invert_rle <- function(chr, 
                            v.snp.pos,
                            gr.reads){
  
  q=GRanges(seqnames=chr,
            ranges=IRanges(start = min(v.snp.pos), end = max(v.snp.pos)),
            strand="*")
  
  gr.reads.sel <- subsetByOverlaps(gr.reads, q)
  
  # remove 0 scores (no reads) and inverse RLE
  df.reads <- as.data.frame(gr.reads.sel)
  df.reads <- subset(df.reads, df.reads$score > 0)
  
  idx <- 1
  v.pos <- numeric(sum(df.reads$width))
  pb <- txtProgressBar(min = 0, max = nrow(df.reads), style = 3)
...
extract_avg_postfrequency <- function(parent.chr,
  ...
  for(i in 1:length(v.chromosomes)){
    ...
    for(j in 1:length(v.bQTLs.pos)){
      ...
      if(j %% n.snps.split == 0){
        v.seq <- v.bQTLs.pos[j:min(length(v.bQTLs.pos), (j + n.snps.split))]
        gr.reads.split <- define_split(v.seq,
                                       chr,
                                       gr.reads.chr, 
                                       th.bp_ranges)
...
blacklisting_asbs <- function(df.bQTLs, 
                              path.raw_input_reads,
                              n.chromosomes = 10,
                              n.snps.split = 50,
                              th.bp_ranges = 500){
  ...
  for(i in 1:n.chromosomes){
    ...
    chr.Mo17 <- paste("Mo17-chr", i, sep = "")
    ## for computational efficiency only - subet to chrB73 equals chr Mo17 (should be enough background samples still)
    df.snps.i <- subset(df.ASBs, df.ASBs$`B73-chr` == chr.B73 & df.ASBs$`Mo17-chr` == chr.Mo17)
    df.bQTLs <- rbind(df.bQTLs, df.snps.i)
  }
  ...
  # saveRDS(df.bQTLs, "df.bQTLs.chr_corresponding.rds")
  ...
  for(i in 1:n.chromosomes){
    ...
    chr.Mo17 <- paste("Mo17-chr", i, sep = "")
    ## for computational efficiency only - subet to chrB73 equals chr Mo17 (should be enough background samples still)
    df.snps.i <- subset(df.bQTLs, df.bQTLs$`B73-chr` == chr.B73 & df.bQTLs$`Mo17-chr` == chr.Mo17)
    
    df.bQTLs.corr <- rbind(df.bQTLs.corr, df.snps.i)
  }
  
  nrow(df.bQTLs.corr) ### 700 weniger
  plot(df.bQTLs.corr$POSTfreq, df.bQTLs.corr$pf.region)
