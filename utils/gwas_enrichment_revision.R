...
gwas_enrichment <- function(df.bQTLs, 
                            df.phenotypic_snps, 
                            v.partitions,
                            n.chromosomes = 10,
  ...
  df.bQTLs[v.traits] <- "no"
  df.bQTLs["dist_to_phenotypic_snp"] <- NA
  
  l.number_per_trait <- numeric(length(v.traits))
  names(l.number_per_trait) <- v.traits
  
  v.chromosomes <- unique(df.phenotypic_snps$chr)
  
  df.bQTLs_w_phenotypes <- c() 
  for(c in v.chromosomes){
    df.phenotypic_snps.chr <- subset(df.phenotypic_snps, df.phenotypic_snps$chr == c)  
    df.bQTLs.chr <- subset(df.bQTLs, df.bQTLs$`B73-chr` == c)
    
    for(t in v.traits){
      df.phenotypic_snps.chr.trait <- subset(df.phenotypic_snps.chr, df.phenotypic_snps.chr$trait == t)
      if(nrow(df.phenotypic_snps.chr.trait) > 0){
        for(j in 1:nrow(df.bQTLs.chr)){
          v.dist <- abs(df.bQTLs.chr$`B73-pos`[j] - df.phenotypic_snps.chr.trait$pos.start)
          j.set <- which(v.dist <= s.dist_to_phenotypic_snp)
          if(length(j.set) > 0){
            l.number_per_trait[t] <- l.number_per_trait[t] + length(j.set)
            df.bQTLs.chr[j, t] <- "yes"
            df.bQTLs.chr$dist_to_phenotypic_snp[j] <- mean(v.dist[j.set])
          }
        }
      }
    }
    df.bQTLs_w_phenotypes <- rbind(df.bQTLs_w_phenotypes, df.bQTLs.chr)
  }
  
  return(list(df.bQTLs_w_phenotypes=df.bQTLs_w_phenotypes, l.number_per_trait=l.number_per_trait))
}



context_gsea <- function(tb.context.sample, 
                         tb.context.population, 
                         sampleSize,
                         popSize,
                         scale_factor = 10,
