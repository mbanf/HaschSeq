...
asb_methylation_frequency_at_distance <- function(df.bQTLs, 
  ...
  for(i in 1:n.chromosomes){
    ...
    df.methylation.i <- subset(df.methylation.i, df.methylation.i$start >= min(df.bQTLs.i[,pos] - th.bp_distance) 
                             & df.methylation.i$start <= max(df.bQTLs.i[,pos]) + th.bp_distance)
    ...
    p.end <- v.pos.bQTLs[min(length(v.pos.bQTLs.i), n.snps.split)] + th.distance_to_ASB
    ...
    for(j in 1:length(v.pos.bQTLs.i)){
      ...
      if(j %% n.snps.split == 0){
        ...
        p.end <- v.pos.bQTLs.i[min(length(v.pos.bQTLs), (j + n.snps.split))] + th.distance_to_ASB
        idx.split <- which(p.start <= v.pos.methylation & v.pos.methylation <= p.end)
        v.pos.methylation.split <- v.pos.methylation[idx.split]
        v.vals.methylation.split <- v.vals.methylation[idx.split]
      }  

      idx <- which(v.pos.methylation.split >= v.pos.bQTLs.i[j] - th.bp_distance & v.pos.methylation.split <= v.pos.bQTLs.i[j] + th.bp_distance)
      if(length(idx) == 0)
        next 
...
methylation_occupancy_distance_to_asb  = function(df.ASBs, 
  ...
  for(k in 1:length(v.filenames)){
    ...
    v.hist.methylation <- asb_methylation_frequency_at_distance(df.ASBs, 
                                                                df.methylation,
    ...
    gc()
    saveRDS(v.hist.methylation, paste(folder, paste("v.hist.methylation.ASBs.",v.parents[k],".", v.variants[k],".0.5.rds", sep = ""), sep ="/"))  
    ...
    v.hist.methylation <- asb_methylation_frequency_at_distance(df.ASBs, 
                                                                ...
                                                                n.snps.split = 50)
    gc()
    saveRDS(v.hist.methylation, paste(folder, paste("v.hist.methylation.ASBs.",v.parents[k],".", v.variants[k],".",th.high_affinity,".rds", sep = ""), sep ="/"))  
  ...
  # TODO: pairs of files for one figure
  ...
  v.variant_sets <- c("CG", "CHG", "CHH")
  
  v.affinity = c("all affinity", "high affinity", "all affinity", "high affinity")
  v.parent_sets = c("B73", "B73", "Mo17", "Mo17")
  
  for(i in 1:length(v.variant_sets)){
    
    v.hist.methylation <- c(readRDS(paste(folder, paste("v.hist.methylation.ASBs.B73.", v.variant_sets[i],".0.5.rds", sep = ""), sep ="/")),
                            readRDS(paste(folder, paste("v.hist.methylation.ASBs.B73.", v.variant_sets[i],".",th.high_affinity,".rds", sep = ""), sep ="/")),
                            readRDS(paste(folder, paste("v.hist.methylation.ASBs.Mo17.", v.variant_sets[i],".0.5.rds", sep = ""), sep ="/")),
                            readRDS(paste(folder, paste("v.hist.methylation.ASBs.Mo17.", v.variant_sets[i],".",th.high_affinity,".rds", sep = ""), sep ="/")))
    ...
    ggplot(df.hist, aes(x=bin, y=val, colour = color, linetype = line_type)) + geom_line() + scale_colour_manual(values = c("green", "blue")) + theme_bw() + xlim(-1990, 1990)
