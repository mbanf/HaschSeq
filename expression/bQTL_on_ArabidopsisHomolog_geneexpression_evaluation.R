bQTL_on_ArabidopsisHomolog_geneexpression_evaluation <- function(df.bQTL_RNAseq, 
  ...
  # here we use phytozome
  
  # Fig. 1. BZR1 regulatory network in maize and Arabidopsis. h) Heatmap of direct orthologues target genes of BZR1 in Arabidopsis and maize
  # induced (green) or repressed (red) by BR. Overall, 65% of 1:1 orthologs and 72% of 1:2 copy
  # orthologs of direct BZR1 targets between Arabidopsis and Maize, respectively, showed the same
  # direction of BR regulation.
  ...
  df.expression_zmays <- merge(df.bQTL_RNAseq, df.ortologs, by = "gene.ID")
